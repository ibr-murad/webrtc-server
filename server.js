...
io.on('connection', socket => {
    ...
    socket.on(ACTIONS.RELAY_SDP, ({peerID, sdp, sdpType}) => {
        console.log('socket RELAY_SDP to ->', peerID);
        console.log('socket RELAY_SDP from ->', socket.id);
    ...
    socket.on(ACTIONS.RELAY_ICE, ({peerID, iceSdp, iceSdpMLineIndex, iceSdpMid}) => {
        console.log('socket RELAY_ICE to ->', peerID);
        console.log('socket RELAY_ICE from ->', socket.id);
        console.log('_______ - - - ______');
    ...
    socket.on(ACTIONS.RELAY_AUDIO_STATE, ({roomID, audioState}) => {
        console.log('socket RELAY_AUDIO_STATE of ->', socket.id);
        const clients = Array.from(io.sockets.adapter.rooms.get(roomID) || []);
    ...
    socket.on(ACTIONS.RELAY_VIDEO_STATE, ({roomID, videoState}) => {
        console.log('socket RELAY_VIDEO_STATE of ->', socket.id);
        const clients = Array.from(io.sockets.adapter.rooms.get(roomID) || []);
